...
#Retrieving the argument that is the issue number
...
# matrix <- read.csv(file="./matrix.csv",head=TRUE,sep=",",stringsAsFactors=F)
# matrix$X<- NULL
...
controlEv <- evtree.control(minbucket = minbucket, minsplit = minsplit, maxdepth = maxdepth,
                            niterations = 1000L, ntrees = ntrees, alpha =0.25,
...
print(Sys.time())
#create the model on the training data (predict X169 in function of the historical window)
...
#save the evtree model, the prediction and the mre in a R object 
nameModel <- paste("./model_evtree_p",numProb,".rda", sep ="")
#nameModel <- paste("./model_evtree_ap",numProb,".rda", sep ="")
...
#nameResult <- paste("./result_ap",numProb,".txt",sep = "")
...
#Save the plot
nameplot <- paste("./evtree_plot_p",numProb,".pdf",sep = "")
#nameplot <- paste("./evtree_plot_ap",numProb,".pdf",sep = "")
pdf(nameplot)
plot(g_evtreeV3)
dev.off()
